...
std::ostream& operator<< (std::ostream& os, Cities& city) {
	...
	while (count < city.coord_vect.size()) {
		...
		std::string curr_int = std::to_string(curr_pair.first);//20 char buffer for any number
		//std::to_string(curr_pair.first, curr_int, 10);//convert int to string
		int char_count = 0;
		...
		os.put(' ');
		curr_int = std::to_string(curr_pair.second);
		char_count = 0;
...
Cities::permutation_t Cities::random_permutation(unsigned len) {//creates a random vector of permutations from 0-len-1
	...
	if (isSRandInit == false) {
		std::srand(time(0));//only sets the random time once
		isSRandInit = true;
	}

	permutation_t out_vect;
	for (long unsigned int i = 0; i < len; i++)out_vect.push_back(i);
	std::shuffle(out_vect.begin(), out_vect.end(), std::default_random_engine(std::chrono::system_clock::now().time_since_epoch().count()));//randomly shuffles the full vector.
	return out_vect;
}
